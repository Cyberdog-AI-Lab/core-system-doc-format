...
> **これは初回導入用**。すでに導入済みのプロジェクトを最新キットへ追従させたいときは [kit-update](../kit-update/SKILL.md) を使う（埋めた実文書を壊さずに更新する）。

クローンした本キットから、**既存のプロジェクトリポジトリ**へ一式を導入する。
コピーは `templates/` を使った機械的な作業、AIがやるのはシステム名の差し込み・CLAUDE.md の追記・`.claude/settings.json` のフック登録（JSONマージ）だけ。**内容の創作はしない**。
...
3. **`.claude/settings.json` へのフック登録**（JSONマージ。cpでは済まない唯一のステップ）：
   - 無ければ、以下をそのまま新規作成する：
     ```json
     {
       "hooks": {
         "PostToolUse": [
           { "matcher": "Edit|Write", "hooks": [
             { "type": "command", "command": "bash .claude/hooks/check-doc-ops-guide-sync.sh", "timeout": 10 }
           ]}
         ]
       }
     }
     ```
   - **既にあれば**、`hooks.PostToolUse` 配列に上記の `matcher: "Edit|Write"` エントリを**追記**する（同じ matcher が既にあれば、その `hooks` 配列に command を追加。既存の他の設定・他イベントは一切変更しない）
   - 追記後は `jq -e '.hooks.PostToolUse[] | select(.matcher=="Edit|Write") | .hooks[] | select(.type=="command") | .command' $DST/.claude/settings.json` で構文とマージ結果を確認する
4. `$DST/$DOCS/README.md` を生成：何が導入されたか・ダッシュボードの起動方法・書く順序・スキルの呼び出しフレーズ
