...
キットの `templates/manifest.json` に、導入先の manifest.json に無い**構造キー**（例：この体系では過去に `skillsDir`・`skills` 配列を追加した）があれば**足す**。ただし `title`・`subtitle`・`docs` の各エントリ値・`instance.file` など**プロジェクト固有の値は保持**する。`docs` 配列の要素が増減（文書の新設・改名）していたら、その差分を提示してマージする。判断に迷う値は上書きせず確認する。
